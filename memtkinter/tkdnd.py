...
to use simply:
    import memtkinter.tkdnd as tkdnd
after importing tkinter or tix or both

This will patch any previously imported tkinter or tix libraries to add
drag and drop methods defined in DnDWrapper.
As such it must be imported after tkinter or tix

This will also add the following constants to the global namespace:
...
def patch_class(cls):
	'''this copys the methods from the DnDWrapper class to the class passed to the function'''
	for method in ['_subst_format_dnd', '_subst_format_str_dnd'] + [x for x, y in DnDWrapper.__dict__.items() if type(y) == FunctionType]:
		setattr(cls, method, getattr(DnDWrapper, method))

def wrapped_init(self, *args, **kwargs):
	'''this allows us to wrap the origional init and call the _require method'''
	self.__originit__(*args, **kwargs)
	self.TkdndVersion = _require(self)

# get a list of loaded modules, this means TkinterDnD should be loaded last
modules = list(sys.modules.keys())
modules_patched = 0
if ('tkinter' in modules) or ('Tkinter' in modules):
	try:
		import tkinter
	except ImportError:
		import Tkinter as tkinter
	patch_class(tkinter.Misc)
	tkinter.Tk.__originit__ = tkinter.Tk.__init__
	tkinter.Tk.__init__ = wrapped_init
	modules_patched += 1

if ('tix' in modules) or ('Tix' in modules):
	try:
		from tkinter import tix
	except ImportError:
		import Tix as tix
	patch_class(tix.Misc)
	tix.Tk.__originit__ = tix.Tk.__init__
	tix.Tk.__init__ = wrapped_init
	modules_patched += 1
	
if modules_patched == 0:
	raise ImportError('TkinterDnD must be imported after the tkinter and tix if used')